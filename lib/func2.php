<?
   ...
   function latest_article($table, $loop, $char_limit, $cnt)    //테이블명, 게시물 개수, 문자개수
   {
		...
		while ($row = mysql_fetch_array($result))   //읽어드린 레코드 만큼..
		{
			...
			if ($len_subject > 50)   //제목의 길이가 지정한 길이보다 크면
			{
				$subject = mb_substr($row[subject], 0,50, 'utf-8');   // 첫번째 문자부터 $char_limit만큼 잘라낸다.
			...
			if($cnt==2){
			   ...
			   $content = $row[content];    // 제목을 저장한다    50자......
				...
				if ($len_content > 100)   //제목의 길이가 지정한 길이보다 크면
				{
					$content = mb_substr($row[content], 0, 100, 'utf-8');   // 첫번째 문자부터 $char_limit만큼 잘라낸다.
													//mb_substr 은 입력받은 문자열을 정해진 길이만큼 잘라서 리턴하는데 
													//2byte 문자인 한글에 대해서도 처리가 가능한 함수입니다. 
			...
			if($cnt==1){
					...
					echo "      
						<li>
							<a href='./$table/view.php?table=$table&num=$num'>
							   <img src='$file_copied' width='50' height='50' alt='datefmt_set_calendar'>	
							   <dl>
